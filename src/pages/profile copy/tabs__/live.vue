<template>
 <div>
    ...
    <div v-if="konten.length > 0" v-for="(item, index) in konten" :key="index" >
      <div>
        <ProfileBeranda :prop_data="item.user" class="q-px-sm q-py-sm" />


        <ItemProfileLayoutVideo
          :index="index"
          :form="{
...
export default {
	computed: {
		// ...mapFields({
		//       konten: "profile.konten_live",
		//       paginate: "profile.paginate_live",
  ...
  watch: {
    ...
    current_tab(val) {
      if(val == 'video') {
        ...
        this.dispatchVuex('profile/action_payload_more', { alias: 'video', page: 1, tag: 'by_paginate'  })
        // this.dispatchVuex('beranda/action_payload', { pull_refresh: true })
      }
    }
  },
	mounted() {
		const tab_toolbar = document.getElementById("toolbar").offsetHeight
		const tab_tab = document.getElementById("tab").offsetHeight

		this.height = "min-height:" + Number(this.$q.screen.height - tab_toolbar - tab_tab) + "px"
	},
