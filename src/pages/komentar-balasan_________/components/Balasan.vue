<template>
	...
	<fragment>
      ...
      <q-item :to="{
          name: 'profile',
          params: {
            id: prop_data.id_user
          }
        }" clickable class="q-pa-sm position-relative" v-ripple="{color:'grey'}">
        <q-item-section top avatar>
          <q-avatar>
             <img :src="img_checker(prop_data.user.foto)" @error="handleError" v-lazy-src="img_checker(prop_data.user.foto)" @load="onLoad" loading="lazy">
        ...
        <q-item-section>
          <q-item-label class="q-mt-sm text-weight-bold">{{ prop_data.user.nama | capitalize }}</q-item-label>
