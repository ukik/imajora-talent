<template>
  ...
  <q-select
    ...
    :options="options"
    @input-value="$emit('onBubbleEvent', null)"
    @filter="filterFn"
  ...
  >
    <template v-slot:label>
      <span class="text-grey-6">Kode Negara</span>
    </template>

    <template v-if="country_selected" v-slot:selected-item="">
      <q-item-label lines="1" class="text-grey-6">
        {{ $filters.capitalize_all(country_selected) }}
      </q-item-label>
    </template>
