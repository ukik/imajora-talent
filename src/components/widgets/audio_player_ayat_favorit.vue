<template>
  <div class="vueAudioBetter" :style="{width: totalWidth}">
    ...
    <div class="operatorButton">
      ...
      <q-btn class="q-mx-sm"
        @click="$emit('onBubbleEvent_Dialog', { index: index, alias: 'deskripsi' })"
        unelevated       
        dense
        flat
        size="18px"
  ...
  watch: {
    curProgress(v) {
      ...
      if(Number(v) >= 50) {
        ...
        if(this.PROGRESS_TRIGGERED) return
        this.PROGRESS_TRIGGERED = true
    
        this.$emit('onBubbleEvent_Dialog', { index: this.index, alias: 'ayat_terakhir_dibaca' })
      }
